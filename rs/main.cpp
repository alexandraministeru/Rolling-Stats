...
class Object
{
	public:
	    ...
	    Object(int n): dataStatus(0), dataTarget(n){ }; //Constructor sets maximum amount of data to be stored
	    Object(const Object& someObject, int qs, int qe): dataStatus(qe-qs+1), dataTarget(someObject.dataTarget)  //Constructor copies a set range of elements to new object to preserve original container
	    {
    ...
    private:
		...
		int dataTarget;         //Maximum number of elements to be stored
		int dataStatus;         //Current number of elements stored
...
{                                                           //removing elements outside the range
    ...
    for (unsigned int p=0; p<query_start-1; p++)
         StoreData.pop_front();
    dataStatus=query_end-query_start+1;
}
